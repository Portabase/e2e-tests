...
test.describe.serial(() => {
    test("Create agent", async ({api}) => {
        agent = await data(await api.post(apiPath("/agents"), {data: {name}}), 201);
        expect(agent).toMatchObject({id: expect.any(String), name});
    });

    test("List, get and retrieve agent key", async ({api}) => {
        expect(await data<any[]>(await api.get(apiPath("/agents")))).toEqual(expect.arrayContaining([
            ...
            expect.objectContaining({id: agent.id, name}),
        ]));
    ...
    test("Reject invalid agent payloads", async ({api}) => {
    ...
    test("Reject unknown agent IDs", async ({api}) => {
